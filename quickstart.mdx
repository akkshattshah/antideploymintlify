...
<Steps>
  <Step title="Get a credential">
    If an agent is doing this, let it run the
    [terminal login](/api/terminal-login) itself. You approve one browser
    prompt, and it creates the project and stores the token without anything
    being copied or pasted.

    Working by hand instead? In the dashboard, choose **New project → API**.
    You get a project API key, shown once, scoped to that one project.
  </Step>
  ...
  <Step title="Push the project directory">
    ...
    An account token has to say which project it is for: add
    `?applicationId=$APP_ID` to the URL. A project key already knows.
...
<Note>
If you're pointing an AI agent at Antideploy, give it
`https://antideploy.com/llms.txt` and nothing else. It explains the whole path
from an empty terminal to a live URL, and links the full contract at
`https://antideploy.com/api/v1`, which is readable without a credential.

You do not need to hand it a key. It can get its own, and a credential pasted
into a chat window stays in that conversation's transcript for good.
</Note>
...
A live HTTPS URL, a history of every deploy, searchable logs, and health checks
that email you if the app goes down.

## Before you rely on it

Antideploy raises **hazards** during analysis for things that will not work
once deployed: files written to local disk, background workers, polling loops.
Read them. They are the difference between "deployed" and "working."
