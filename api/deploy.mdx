...
| 403 | `account_suspended` | The account is suspended. Waiting does not clear it; do not retry. |
...
| 503 | `hosting_paused` | Deploys are paused platform-wide for a while. Retry after `retryAfterSeconds`. |
| 503 | `uploads_paused` | Pushed folders cannot be stored right now. Retry after `retryAfterSeconds`. |
