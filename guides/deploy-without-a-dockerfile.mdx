...
* **Files written to local disk do not persist.** The filesystem is ephemeral
  ...
  deploy.
* **Background workers are not started for you.** The container sleeps when
  idle, so a worker or a polling loop will not stay alive. Scheduled work is
  the exception: cron jobs are supported and call a path on your own app.
