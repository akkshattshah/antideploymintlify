...
<Steps>
  ...
  <Step title="Release">
    The image replaces the previous version on your app's single instance, and
    your environment variables are injected. The deploy counts as live only once
    the app answers on its port. If it never does, the deploy fails, and since
    the previous version has already been replaced, recovering means deploying a
    version that starts.

    Your app is then reachable over HTTPS at its own address on `antideploy.com`.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Hazards" icon="circle-exclamation">
    ...
    lost on restart. Background workers and polling loops that will not stay
    alive on a container that sleeps.
  </Card>
</CardGroup>
