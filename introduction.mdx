...
<Steps>
  ...
  <Step title="Release">
    The container replaces the previous version and is given an HTTPS URL.
    Environment variables are injected at this point, and the deploy counts as
    live only once the new version answers.
  </Step>
</Steps>
...
- **No domains of your own.** Apps are published at `yourapp.antideploy.com`.
- **No long-running background workers.** Each app is one HTTP container that
  sleeps when idle, so a worker or a polling loop will not stay alive.
  Antideploy tells you at analysis time when your project defines one.
- **Cron is supported.** Up to three scheduled jobs per app, five minute
  minimum interval. Each one calls a path on your own app.
...
<CardGroup cols={2}>
  ...
  <Card title="Deployments" icon="clock-rotate-left" href="/operations/deployments">
    Full history of every release, with the commit each GitHub deploy built.
  </Card>
